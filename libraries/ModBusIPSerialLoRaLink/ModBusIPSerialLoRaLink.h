...
class ModbusIPSerialLoRaLink : public Modbus {
    private:
        ...
        int send_cmd_lora(char * cmd, bool printCmd, bool printRes, bool doubleRes);
        char hexToC(char c);
        char hexToChar(char c1, char c2);

    public:
        ModbusIPSerialLoRaLink();
        void update();
        ...
        void configLoRa(HardwareSerial* LoRaPort,
                           long baud,
                           long _timeout,
                           unsigned char resetpin);
};
